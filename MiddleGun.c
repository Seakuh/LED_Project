...
SimplePatternList gPatterns = { startMiddleGun, startGun, colorPunch, sound , sinelon, juggle, bpm   };
...
int colorPunchCounter = 2;

// colorPunch -------------------------------------------------------------------------------

//Immer in goesseren Abstaenden rot blinken und dann komplett rot

void colorPunch() {
  for ( int k = 0; k < 20; k++) {
    FastLED.delay(200);
    for ( int i = 0; i < NUM_LEDS; i++) {
      if (i % colorPunchCounter == 0) {
        leds[i] = CRGB::Red;
      }
